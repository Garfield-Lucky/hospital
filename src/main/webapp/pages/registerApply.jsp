...
<div class="mask"></div>
     ...
     <div class="registerPage registerInto"  data-id='${userinfo.id}' style="top: 316.5px;">
       ...
       <div class="context">
              <span>用户id：
                   <input name="userId" type="text"  placeholder="用户id" class="userId">
              </span>
       </div>
        <div class="payType">
              <span>支付方式：
                  <input type="radio" name="payType" value="0" class="marry" checked="checked">现金
			      <input type="radio" name="payType" value="1" class="marry">账户余额
              </span>
       </div>
...
<script src="js/register/registerApply.js"></script>
...
{{#each this}}
 <tr>
  ...
  <td>{{officeType}}</td>
  <td>{{title}}</td>
  <td>{{workDate}} ({{workTime}})</td>
  <td>{{clinicType}}</td>
  <td>{{reaminCount}}</td>
  <td>{{registerFee}}</td>
  <td><span data-code={{doctorCode}}   data-date={{trueDate}} data-office={{officeCode}} class="register">挂号</span> </td>
 </tr>
 {{/each}}
</script>  
</body>
