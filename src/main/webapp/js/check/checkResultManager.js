...
$(function(){
	 $(".query").find('.find').click(function(){
		 ...
		 $.ajax({
				type : "POST",
				url : "check/findCheckResultByInput.do",
				async:false,
				data : {
					'vistId':vistId,
					'userId':userId,
	 ...
	 $("#session1").on('click','.update',function(){
		 ...
		 var checkResult = $(this).attr('data-content');
		 var result = $(this).attr('data-content');
		 var userId = $(this).attr('data-userId');
		 var userName = $(this).attr('data-userName');
 		 $('.mainbody_right').load("check/showUpdate.do",{checkResult:checkResult,id:id,userId:userId,userName:userName});
 	     });
...
function pageA(state,pageTo){
 ...
 function checkList() {
		...
		$.ajax({
			type : "POST",
			url : "check/getCheckResultList.do",
			async:false,
			data : {
				'page':page,
				'pageSize':pageSize
